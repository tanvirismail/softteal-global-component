...
export function getProducts() {
  return async () => {
    ...
    try {
      const response: { data: { products: Product[] } } = await axios.get(process.env.MINIMAL_API_KEY+'/api/products');
...
export function getProduct(name: string) {
  return async () => {
    dispatch(slice.actions.startLoading());
    try {
      const response: { data: { product: Product } } = await axios.get(process.env.MINIMAL_API_KEY+'/api/products/product', {
        params: { name },
      });
